...
export function useMapInstance({
  ...
  useEffect(() => {
    ...
    const reportRuntimeError = (error: unknown) => {
      const nextError = error instanceof Error ? error : new Error("Map failed to initialize.");
      queueMicrotask(() => {
        if (!disposed) setRuntimeError(nextError);
      });
    };
  ...
  return { mapRef, isStyleLoaded, styleLoadCount, runtimeError };
}
